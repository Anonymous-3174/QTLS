...
def run_simulation_single_pulse_full(
    ...
    # H_int = sum(
    #             interactions[i, j] * (sx[i] * sx[j])
    #             for i in range(N_tls) for j in range(i)
    #         )
    H_int = sum(
        interactions[i, j] for i in range(N) for j in range(i)
    )  # Dipole-Dipole Interaction
    ...
    result = mesolve(H, psi0, tlist, c_ops, e_ops=e_ops, options={"progress_bar": None})
...
def run_simulation_double_pulse(
    ...
    return np.real_if_close(
        ...
        mesolve(
            H, psi0, tlist, c_ops, e_ops=[Sm.dag() * Sm], options={"progress_bar": None}
        ).expect[0]
    )
...
def build_spin_spin_interactions_random_distribution(
    N_tls, J_min, J_max, alpha_x=1.0, alpha_y=1.0, alpha_z=0.5
):
