...
def build_dataset(data_dir: str, output_dir: str, write_csv_file: bool = True):
    ...
    print(
        f"Total rows      : {n_temps} x {n_freqs} x {n_spacings} x {n_phases} x "
        ...
        for freq_mhz in FREQUENCY_LIST_MHZ:
            for spacing_ns in SPACING_LIST_NS:
                ...
                if IQ_matrix.shape != (2, n_phases, N_TIME_SAMPLES):
                    print(
                        f"  WARNING: {filename} has shape {IQ_matrix.shape}, "
                ...
                if not np.array_equal(phase_array, EXPECTED_PHASE_DEG):
                    print(
                        f"  WARNING: {filename} pulse_phase_array does not match "
                ...
                for phase_idx, phase_deg in enumerate(phase_array):
                    ...
                    data_all[row_idx:end_idx, COL["I"]] = I_matrix[
                        phase_idx
                    ]  # all 1000
                    data_all[row_idx:end_idx, COL["Q"]] = Q_matrix[
                        phase_idx
                    ]  # all 1000
    ...
    if row_idx != n_total_rows:
        ...
        print(
            f"  [PASS] Row count: {row_idx:,} = {n_temps} temps x {n_freqs} freqs "
